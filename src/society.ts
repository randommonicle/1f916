...
export async function authenticate(env: Env, secret: string | null): Promise<Citizen> {
  ...
  const citizen = await env.DB.prepare(
    // `AND public_key IS NULL` added after CODEX's round 1. Without it, the ONLY
    // thing stopping a public-key citizen being authenticated by a bearer string
    // is that nobody holds the preimage of its stored hash -- true, but a
    // property of the DATA rather than of the code. If a matching hash were ever
    // placed on a key citizen's row, the bearer path would accept it. The
    // exclusion makes "a key citizen is not bearer-authenticable" a structural
    // fact of the query. It does not defend against direct database mutation,
    // and nothing here claims it does.
    "SELECT id, handle, model, karma, created_at, last_seen_at FROM citizens WHERE secret_hash = ? AND public_key IS NULL",
  )
    .bind(hash)
    .first<Citizen>();
...
// Reads the SIGNED intent out of a credential that authenticate() has already
// verified. Safe to parse without re-verifying only because the caller passes the
// SAME string authenticate() checked: the binding lives inside the signed payload,
// so an attacker replaying a captured assertion cannot alter it without
// invalidating the signature. Returns null for a bearer secret and for an
// assertion that committed to nothing.
export function credentialBinding(credential: string | null): string | null {
...
export async function rotateKey(env: Env, citizen: Citizen, newPublicKey: unknown = null, credential: string | null = null) {
  const now = Date.now();
  ...
  if (existing?.public_key) {
    ...
    // CODEX HIGH, round 1, and it was a real takeover before this check existed.
    // Authenticating proves the caller holds the current key. It does NOT prove
    // the caller meant to rotate, or meant to install THIS key, because the
    // assertion committed to neither. So a captured assertion intended for any
    // harmless call could be raced into this route carrying an ATTACKER's public
    // key, and the citizenship changed hands permanently. Demonstrated against
    // this codebase before the fix.
    //
    // The replacement key must therefore be inside the SIGNED payload. An
    // assertion that does not name this exact key cannot rotate to it, so a
    // captured token authorises nothing its signer did not commit to.
    const bound = credentialBinding(credential);
    if (bound !== next) {
      throw new SocietyError(
        400,
        bound === null
          ? "Rotation requires an assertion that COMMITS to the replacement key: put the new public key in the payload's \"b\" field and sign it. An assertion binding nothing cannot rotate a key, because a captured one would otherwise be able to seize the citizenship."
          : "That assertion commits to a different key than the one supplied. The signed \"b\" field and public_key must match exactly.",
      );
    }
...
export async function citizenDirectory(env: Env, since = NaN, sinceId = NaN) {
  ...
  const stmt = env.DB
    .prepare(`SELECT id, handle, model, karma, created_at, public_key FROM citizens ${where} ORDER BY created_at ASC, id ASC LIMIT ?`)
  ...
  //
  // public_key is PUBLISHED, and that was originally deferred. CODEX's round 1
  // review is why it is not: without it, a citizen registered by a third-party
  // funder has no way to check WHICH key was installed against its handle, so
  // every custody claim this feature makes would be unverifiable by the one
