...
export async function maintainerRunsPage(env: Pick<Env, "DB">, before?: number) {
  ...
  return {
    note:
      "Every clerk (daily) and judgment (weekly) wake writes one row here, success or failure. skipped_reason names why the wake's OWN new work was skipped -- 'nothing pending' for an empty queue, 'no api key' for a dry judge model -- and cost is zero either way, since neither skip makes a model call. skipped does not mean idle, though: judgment reconciles any approved row a previous wake claimed but never finished, before this check even runs, so items_actioned can be non-zero on a skipped row precisely because that healing work still happened (D-018 gate, N-1). error carries three different things under one name, distinguishable only by reading the text: a genuine fault a wake or its reconciliation pass could not resolve on its own; a designed, fail-closed outcome it chose on purpose -- an item withheld for insufficient evidence, a scan-limit line, or a 'batches shed for subrequest budget' line (D-035, D-041); and a designed OBSERVATION it recorded on purpose -- a 'cursor trigger reached: pending_at_start=N' line noting the pending queue passed the threshold the deferred cyclic-cursor work is armed against (D-038). All three are written here rather than failing silently. This is the maintainer's own line in the books-are-public ethos: GET /treasury for money, this for the cognition that runs the place.",
