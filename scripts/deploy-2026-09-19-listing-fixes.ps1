...
#   powershell -ExecutionPolicy Bypass -File ".\scripts\deploy-2026-09-19-listing-fixes.ps1"
...
if ($level -notmatch "main\.\.\.origin/main$") { Stop-Here "push first: the deploy must ship the same commit the public fork carries (R-4 / D-006)." }
...
$colsBefore = npx wrangler d1 execute commonhold --remote --json --command "PRAGMA table_info(listings)" | ConvertFrom-Json
...
$colsAfter = npx wrangler d1 execute commonhold --remote --json --command "PRAGMA table_info(listings)" | ConvertFrom-Json
...
$payingRows = npx wrangler d1 execute commonhold --remote --json --command "SELECT id FROM listings WHERE status = 'paying'" | ConvertFrom-Json
