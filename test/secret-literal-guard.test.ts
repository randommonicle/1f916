...
test("secret-literal guard: every 'secret' literal in src/ is a reviewed wire token or an allowlisted prose entry, none unreviewed", () => {
  ...
  assert.equal(total, 69, "secret-literal total drifted from the committed baseline (69)");
  assert.equal(wire, 23, "exact wire-token count drifted from the committed baseline (23)");
  assert.equal(prose, 46, "prose-literal count drifted from the committed baseline (46)");
});
